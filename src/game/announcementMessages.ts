...
export const ANNOUNCEMENTS = {
  ...
  wildPitchStay: {
    clear: { ...message('폭투가 나왔지만 진루하지 않았습니다.', '명백히 진루 가능한 찬스를 놓쳤습니다.', 'negative', 'wild-pitch-clear'), advance: 'blocked' as const },
  ...
  wildPitchAdvance: {
    ...
    ambiguous: {
      title: '{destination} 진루 성공!',
      detail: '상대 폭투를 틈타 위험을 감수하고 {destination} 추가 진루에 성공했습니다.',
      homeDetail: '상대 폭투를 틈타 위험을 감수하고 {destination}에 들어왔습니다.',
      advance: 'bold' as const,
      detailScene: 'advance-ambiguous-safe' as const,
  ...
  groundThrowSuccessAtBase: (base: 1 | 2 | 3) => shareDetailScene(message(
    `상대 내야수가 ${base}루 송구에 성공했습니다!`,
    base === 1 ? '타자 주자를 1루에서 처리합니다.' : `선행 주자를 ${base}루에서 처리합니다.`,
  ), base === 1 ? 'ground-first-base-catch' : 'out-ground-force'),
  ...
  followUpGroundOut: (playerBase: number | null, outs: number, playerIsForced: boolean) => {
    ...
    if (outs === 3) return ANNOUNCEMENTS.groundThrowSuccess
  ...
  groundThrowingErrorAdvance: {
    clear: {
      ...
      detail: '송구 실책을 이용해 {destination}에 안전하게 도착했습니다.',
      category: 'normal' as const,
      advance: 'normal' as const,
      detailScene: 'advance-clear' as const,
      titleImageMode: 'same-as-detail-scene' as const,
    },
    ambiguous: {
      title: '내야 땅볼 송구 실책 추가 진루 성공!',
  ...
  groundDoublePlay: () => message('내야 땅볼 병살!', '1루 주자와 타자 주자가 모두 아웃되었습니다.', 'negative'),
  followUpBattingEvent: (title: string, before: number | null, playerBase: number | null, isForcedWalk = false) => {
