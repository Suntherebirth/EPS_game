...
const followUpGroundThrowOutcomes = (fieldingError: number) => {
  const throwingError = RUNNING_CHANCES.infieldGroundThrowingError / (1 - fieldingError)
  const throwSafe = (1 - throwingError) * RUNNING_CHANCES.infieldGroundThrowSafe
  return {
    throwOut: (1 - throwingError) - throwSafe,
    throwSafe,
    clearThrowingError: throwingError * RUNNING_CHANCES.infieldGroundThrowingErrorClear,
    ambiguousThrowingError: throwingError * RUNNING_CHANCES.infieldGroundThrowingErrorAmbiguous,
  }
}
...
export const EMPTY_BASES_SINGLE_NODES: Record<string, ScenarioNode> = {
  ...
  'runner.first.clearDrop.decide': {
    ...
    choices: [
      { id: 'stayFirst', label: '안전하게 1루에 머문다', transition: { to: 'runner.route', effects: [{ type: 'record', message: PLAY_RESULT_ITEMS.advanceSecondMissed }, { type: 'announce', title: '외야수 실책이 나왔지만 진루하지 않았습니다.', detail: '명백히 진루 가능한 찬스를 놓쳤습니다.', tone: 'negative', advance: 'blocked', scene: 'error-outfield-through-clear' }] } },
      { id: 'advanceSecond', label: '2루로 진루한다', description: '완전히 뒤로 빠진 타구 · 성공률 100%', transition: { to: 'runner.route', effects: [{ type: 'movePlayer', to: 2 }, { type: 'record', message: PLAY_RESULT_ITEMS.advanceSecondError }, { type: 'announce', title: '2루 진루 성공!', detail: '외야수 실책을 이용해 2루에 안전하게 도착했습니다.', category: 'normal', advance: 'normal', detailScene: 'advance-clear', titleImageMode: 'same-as-detail-scene' }] } },
    ],
  },
  ...
  'runner.first.ambiguousDrop.advance': {
    ...
    title: '2루 진루',
    outcomes: [
      { id: 'success', label: '2루 진루 성공', weight: RUNNING_CHANCES.advanceOnAmbiguousDrop, transition: { to: 'runner.route', effects: [{ type: 'movePlayer', to: 2 }, { type: 'record', message: PLAY_RESULT_ITEMS.advanceSecondETB }, { type: 'announce', title: '2루 진루 성공!', detail: '상대 외야수 실책을 틈타 위험을 감수하고 2루 추가 진루에 성공했습니다.', advance: 'bold', detailScene: 'advance-ambiguous-safe', titleImageMode: 'same-as-detail-scene' }] } },
      { id: 'out', label: '2루 진루 실패', weight: 1 - RUNNING_CHANCES.advanceOnAmbiguousDrop, transition: { to: 'plate.complete', effects: [{ type: 'movePlayer', to: 'out' }, { type: 'record', message: PLAY_RESULT_ITEMS.advanceSecondFailure }, { type: 'announce', title: '2루 진루 실패', detail: '상대 외야수의 정확한 송구로 2루에서 아웃되었습니다.', tone: 'negative', detailScene: 'advance-ambiguous-out', titleImageMode: 'same-as-detail-scene' }] } },
    ],
  },
  'runner.route': {
    id: 'runner.route',
  ...
  'runner.wildPitch.clear.decide': {
    ...
    choices: [
      { id: 'stayOnBase', label: '안전하게 현재 베이스에 머문다', transition: { to: 'runner.route', effects: [{ type: 'record', message: '{destination} 진루 기회 놓침' }, { type: 'announce', ...ANNOUNCEMENTS.wildPitchStay.clear }] } },
      { id: 'advance', label: SCENARIO_TEXT.advanceChoice, description: '폭투를 이용한다 · 성공률 100%', transition: { to: 'runner.route', effects: [{ type: 'record', message: '{destination} 진루 (상대 실책)' }, { type: 'advancePlayer' }, { type: 'announcePlayerAdvance', ...ANNOUNCEMENTS.wildPitchAdvance.clear }] } },
    ],
  },
  ...
  'runner.wildPitch.ambiguous.advance': {
    ...
    title: '폭투 진루 판정',
    outcomes: [
      { id: 'success', label: '폭투 진루 성공', weight: RUNNING_CHANCES.advanceOnAmbiguousWildPitch, transition: { to: 'runner.route', effects: [{ type: 'record', message: '{destination} 진루 (ETB)' }, { type: 'advancePlayer' }, { type: 'announcePlayerAdvance', ...ANNOUNCEMENTS.wildPitchAdvance.ambiguous }] } },
  ...
  'runner.third.outfieldError.clear.decide': {
    ...
    choices: [
      { id: 'stayThird', label: '안전하게 3루에 머문다', transition: { to: 'runner.route', effects: [{ type: 'record', message: PLAY_RESULT_ITEMS.homeAdvanceMissed }, { type: 'announce', title: '외야수 실책이 나왔지만 홈으로 진루하지 않았습니다.', detail: '명백히 진루 가능한 찬스를 놓쳤습니다.', tone: 'negative' }] } },
      { id: 'advanceHome', label: '홈으로 진루한다', description: '명백하게 뒤로 빠진 타구 · 성공률 100%', transition: { to: 'runner.route', effects: [{ type: 'advancePlayer' }, { type: 'record', message: PLAY_RESULT_ITEMS.advanceHomeError }, { type: 'announcePlayerAdvance', title: '홈 진루 성공!', detail: '외야수 실책을 이용해 홈에 안전하게 들어왔습니다.', category: 'normal' }] } },
    ],
  },
  ...
  'runner.third.sacrificeFly.deep.decide': {
    ...
    description: '3루 주자: 태그업 여부를 선택하세요.',
    choices: [
      { id: 'stayThird', label: '3루에 머무른다', transition: { to: 'runner.route', effects: [{ type: 'applySacrificeFlyOut', score: false }, { type: 'record', message: PLAY_RESULT_ITEMS.homeAdvanceMissed }, { type: 'announce', title: '명백하게 깊은 외야 플라이, 3루에 머뭅니다.', detail: '명백히 진루할 수 있는 타구였지만 태그업하지 않았습니다.', tone: 'negative' }] } },
  ...
  'followUp.ground.contact.check': {
    id: 'followUp.ground.contact.check', tags: ['composite-event-step'],
    type: 'chance',
    view: 'batter',
  ...
  'followUp.ground.soft.check': {
    id: 'followUp.ground.soft.check', tags: ['composite-event-step'], type: 'chance', view: 'batter', title: '후속 약한 땅볼 포구 판정',
    outcomes: [
      { id: 'fieldingError', label: '내야수 포구 실책', weight: RUNNING_CHANCES.infieldGroundSoftFieldingError, transition: { to: 'followUp.ground.fieldingError' } },
      { id: 'cleanPlay', label: '내야수 포구 성공', weight: 1 - RUNNING_CHANCES.infieldGroundSoftFieldingError, transition: { to: 'followUp.ground.advanceOpportunity.route', effects: [{ type: 'setFlag', key: 'groundContact', value: 'soft' }, { type: 'announce', ...ANNOUNCEMENTS.groundFieldingSuccess }] } },
    ],
  },
  'followUp.ground.advanceOpportunity.route': {
    ...
    routes: [
      ...
      { to: 'runner.third.groundOut.decide', when: [{ field: 'playerBase', operator: 'eq', value: 3 }, { field: 'outs', operator: 'lt', value: 3 }], effects: [{ type: 'announce', title: '상대 내야수, 1루 송구 준비 완료!', detail: '3루 주자는 위험을 감수하고 송구 시점에 맞춰 홈 쇄도를 시도할 수 있습니다.', tone: 'caution', detailScene: 'ground-throw-ready', titleImageMode: 'same-as-detail-scene' }] },
      { to: 'followUp.ground.throw.check', effects: [{ type: 'setFlag', key: 'groundAdvanceIntent', value: 'none' }] },
    ],
  },
  'followUp.ground.throw.check': {
    id: 'followUp.ground.throw.check', type: 'router', view: 'batter', title: '후속 내야 땅볼 송구 강도 확인',
    routes: [
      { to: 'followUp.ground.hard.throw.check', when: [{ field: 'flag', operator: 'eq', key: 'groundContact', value: 'hard' }] },
      { to: 'followUp.ground.soft.throw.check' },
    ],
  },
  'runner.second.groundOut.decide': {
    ...
    description: '2루 주자: 주루 방침을 선택하세요.',
    choices: [
      { id: 'staySecond', label: '안전하게 2루에 머문다', transition: { to: 'followUp.ground.throw.check', effects: [{ type: 'setFlag', key: 'groundAdvanceIntent', value: 'stay' }, { type: 'announce', title: '2루에 머무릅니다.', detail: '내야수의 송구 결과를 지켜봅니다.', detailScene: 'ground-throw-ready', titleImageMode: 'same-as-detail-scene' }] } },
      { id: 'advanceThird', label: '내야수 송구 순간 3루 진루를 시도한다', description: '송구 성공 시 성공률 판정', transition: { to: 'followUp.ground.throw.check', effects: [{ type: 'setFlag', key: 'groundAdvanceIntent', value: 'attempt' }, { type: 'announce', title: '3루 진루를 시도합니다.', detail: '내야수의 송구 결과에 따라 진루 성공률이 결정됩니다.', detailScene: 'ground-throw-ready', titleImageMode: 'same-as-detail-scene' }] } },
    ],
  },
  'runner.second.groundOut.advance': {
    ...
    title: '3루 진루',
    outcomes: [
      { id: 'success', label: '3루 진루 성공', weight: RUNNING_CHANCES.advanceOnGroundBallToThird, transition: { to: 'runner.route', effects: [{ type: 'movePlayer', to: 3 }, { type: 'record', message: PLAY_RESULT_ITEMS.advanceThirdETB }, { type: 'announce', ...SCENARIO_TEXT.running.advanceThirdSuccess, title: '후속 타자 내야 땅볼 3루 진루 성공!', advance: 'bold', detailScene: 'advance-ambiguous-safe', titleImageMode: 'same-as-detail-scene' }] } },
      { id: 'out', label: '3루 진루 실패', weight: 1 - RUNNING_CHANCES.advanceOnGroundBallToThird, transition: { to: 'plate.complete', effects: [{ type: 'movePlayer', to: 'out' }, { type: 'record', message: PLAY_RESULT_ITEMS.advanceThirdFailure }, { type: 'announce', title: '후속 타자 내야 땅볼 3루 진루 실패', detail: '상대 내야수의 빠른 중계 플레이로 3루에서 아웃되었습니다.', tone: 'negative', detailScene: 'advance-ambiguous-out', titleImageMode: 'same-as-detail-scene' }] } },
    ],
  },
  'runner.third.groundOut.decide': {
    ...
    choices: [
      { id: 'stayThird', label: '안전하게 3루에 머문다', transition: { to: 'followUp.ground.throw.check', effects: [{ type: 'setFlag', key: 'groundAdvanceIntent', value: 'stay' }, { type: 'announce', title: '3루에 머무릅니다.', detail: '내야수의 송구 결과를 지켜봅니다.', detailScene: 'ground-throw-ready', titleImageMode: 'same-as-detail-scene' }] } },
      { id: 'advanceHome', label: '홈으로 쇄도한다', description: '송구 성공 시 성공률 판정', transition: { to: 'followUp.ground.throw.check', effects: [{ type: 'setFlag', key: 'groundAdvanceIntent', value: 'attempt' }, { type: 'announce', title: '홈 쇄도를 시도합니다.', detail: '상대 내야수가 송구하는 순간 홈으로 쇄도합니다.', detailScene: 'ground-home-rush', titleImageMode: 'same-as-detail-scene' }] } },
    ],
  },
  'runner.third.groundOut.advance': {
    id: 'runner.third.groundOut.advance', type: 'chance', view: 'runner:third', title: '홈 쇄도',
    outcomes: [
      { id: 'success', label: '홈 쇄도 성공', weight: RUNNING_CHANCES.advanceOnGroundBallToThird, transition: { to: 'runner.route', effects: [{ type: 'movePlayer', to: 'home' }, { type: 'record', message: PLAY_RESULT_ITEMS.homeAdvanceETB }, { type: 'announce', ...SCENARIO_TEXT.running.homeAdvanceSuccess, advance: 'bold', detailScene: 'home-in-positive', titleImageMode: 'same-as-detail-scene' }] } },
      { id: 'out', label: '홈 쇄도 실패', weight: 1 - RUNNING_CHANCES.advanceOnGroundBallToThird, transition: { to: 'plate.complete', effects: [{ type: 'movePlayer', to: 'out' }, { type: 'record', message: PLAY_RESULT_ITEMS.homeAdvanceFailure }, { type: 'announce', title: '후속타자의 내야 땅볼 중 홈 쇄도 실패', detail: '송구를 받은 상대 1루수가 재빠르게 홈으로 송구합니다.\n추가진루를 시도하던 3루 주자도 홈에서 아웃되었습니다.', tone: 'negative', scene: 'home-advance-failure', detailScene: 'home-advance-failure', titleImageMode: 'same-as-detail-scene' }] } },
    ],
  },
  ...
  'followUp.ground.hard.throw.check': {
    id: 'followUp.ground.hard.throw.check',
    type: 'chance',
    view: 'batter',
  ...
  'followUp.ground.soft.throw.check': {
  ...
  'followUp.ground.throw.error.attempt.clear': {
    ...
    effects: [{ type: 'forceWalk' }, { type: 'announce', ...ANNOUNCEMENTS.followUpGroundThrowingErrorExtraAdvanceAttempt.clear }, { type: 'advancePlayer' }, { type: 'announcePlayerAdvance', ...ANNOUNCEMENTS.followUpGroundThrowingErrorExtraAdvance.clear }],
  ...
  'followUp.ground.throw.extra.clear.attempt.route': {
    id: 'followUp.ground.throw.extra.clear.attempt.route', type: 'router', view: 'result', title: '송구 실책 추가 진루 확인',
    routes: [
      { to: 'plate.complete', when: [{ field: 'playerBase', operator: 'eq', value: null }] },
      { to: 'followUp.ground.throw.extra.clear.attempt.decide' },
    ],
  },
  ...
  'followUp.ground.throw.extra.clear.decide': {
    ...
    choices: [
      { id: 'stayOnBase', label: '안전하게 현재 베이스에 머문다', transition: { to: 'runner.route', effects: [{ type: 'record', message: '{destination} 진루 기회 놓침' }, { type: 'announce', title: '내야 땅볼 송구 실책이 나왔지만 진루하지 않았습니다.', detail: '명백히 진루 가능한 찬스를 놓쳤습니다.', tone: 'negative', advance: 'blocked', detailScene: 'error-first-base-catch-clear', titleImageMode: 'same-as-detail-scene' }] } },
  ...
  'followUp.ground.throw.extra.clear.advance': {
    ...
    outcomes: [
      { id: 'success', label: '추가 진루 성공', weight: FOLLOW_UP_GROUND_THROW_CLEAR_EXTRA_ADVANCE, transition: { to: 'runner.route', effects: [{ type: 'record', message: '{destination} 진루 (상대 실책)' }, { type: 'advancePlayer' }, { type: 'announcePlayerAdvance', ...ANNOUNCEMENTS.followUpGroundThrowingErrorExtraAdvance.clear }] } },
  ...
  'followUp.ground.throw.extra.clear.attempt.decide': {
    id: 'followUp.ground.throw.extra.clear.attempt.decide', type: 'choice', view: 'runner:first', title: '송구 실책 추가 진루', tags: ['surprise-event', 'player-position-view'],
    description: SCENARIO_TEXT.advanceQuestion,
    choices: [
      { id: 'stayOnBase', label: '안전하게 현재 베이스에 머문다', transition: { to: 'runner.route', effects: [{ type: 'record', message: '{destination} 진루 기회 놓침' }, { type: 'announce', title: '송구 실책이 발생했지만 추가 진루하지 않았습니다.', detail: '추가 진루하지 않고 현재 베이스에 머뭅니다.', tone: 'neutral', advance: 'blocked' }] } },
      { id: 'advance', label: SCENARIO_TEXT.advanceAttemptChoice, description: '명백하게 빠진 송구 · 위험 진루 성공 판정', transition: { to: 'followUp.ground.throw.extra.clear.attempt.advance' } },
    ],
  },
  'followUp.ground.throw.extra.clear.attempt.advance': {
    id: 'followUp.ground.throw.extra.clear.attempt.advance', type: 'chance', view: 'runner:first', title: '송구 실책 추가 진루', tags: ['player-position-view'],
    outcomes: [
  ...
  'followUp.ground.throw.extra.ambiguous.advance': {
    ...
    outcomes: [
      { id: 'success', label: '추가 진루 성공', weight: FOLLOW_UP_GROUND_THROW_AMBIGUOUS_EXTRA_ADVANCE, transition: { to: 'runner.route', effects: [{ type: 'record', message: '{destination} 진루 (ETB)' }, { type: 'advancePlayer' }, { type: 'announcePlayerAdvance', ...ANNOUNCEMENTS.followUpGroundThrowingErrorExtraAdvance.ambiguous }] } },
  ...
  'followUp.ground.throwingError.clear.decide': {
    ...
    description: '주루 방침을 선택하세요.',
    choices: [
      { id: 'stayOnBase', label: '안전하게 현재 베이스에 머문다', transition: { to: 'runner.route', effects: [{ type: 'record', message: '{destination} 진루 기회 놓침' }, { type: 'announce', title: '내야 땅볼 송구 실책이 나왔지만 진루하지 않았습니다.', detail: '명백히 진루 가능한 찬스를 놓쳤습니다.', tone: 'negative', detailScene: 'error-first-base-catch-clear', titleImageMode: 'same-as-detail-scene' }] } },
      { id: 'advance', label: SCENARIO_TEXT.advanceChoice, description: '1루수 뒤로 완전히 빠진 송구 · 성공률 100%', transition: { to: 'runner.route', effects: [{ type: 'record', message: '{destination} 진루 (상대 실책)' }, { type: 'advancePlayer' }, { type: 'announcePlayerAdvance', ...ANNOUNCEMENTS.groundThrowingErrorAdvance.clear }] } },
    ],
  },
  'followUp.ground.throwingError.ambiguous.decide': {
    ...
    description: '주루 방침을 선택하세요.',
    choices: [
      { id: 'stayOnBase', label: '안전하게 현재 베이스에 머문다', transition: { to: 'runner.route', effects: [{ type: 'record', message: '송구 실책, 추가 진루하지 않음', showInCompletion: false }, { type: 'announce', title: '내야 땅볼 송구 실책이 나왔지만 진루하지 않았습니다.', detail: '위험하다고 판단하여 진루하지 않았습니다.', tone: 'neutral', detailScene: 'error-first-base-catch-ambiguous', titleImageMode: 'same-as-detail-scene' }] } },
  ...
  'followUp.ground.throwingError.ambiguous.advance': {
    ...
    outcomes: [
      { id: 'success', label: '추가 진루 성공', weight: RUNNING_CHANCES.advanceOnAmbiguousDrop, transition: { to: 'runner.route', effects: [{ type: 'record', message: '{destination} 진루 (ETB)' }, { type: 'advancePlayer' }, { type: 'announcePlayerAdvance', ...ANNOUNCEMENTS.groundThrowingErrorAdvance.ambiguous }] } },
  ...
  'runner.battingAdvance.clearDrop.decide': {
    ...
    description: '주루 방침을 선택하세요.',
    choices: [
      { id: 'stayOnBase', label: '안전하게 현재 베이스에 머문다', transition: { to: 'runner.route', effects: [{ type: 'record', message: PLAY_RESULT_ITEMS.homeAdvanceMissed }, { type: 'announce', title: '후속 타자 타구 외야수 실책이 나왔지만 진루하지 않았습니다.', detail: '명백히 진루 가능한 찬스를 놓쳤습니다.', tone: 'negative', scene: 'error-outfield-through-clear' }] } },
      { id: 'advance', label: SCENARIO_TEXT.advanceChoice, description: '완전히 뒤로 빠진 타구 · 성공률 100%', transition: { to: 'runner.route', effects: [{ type: 'record', message: '{destination} 진루 (상대 실책)' }, { type: 'advancePlayer' }, { type: 'announcePlayerAdvance', title: '외야수 실책 추가 진루 성공!', detail: '외야수 실책을 이용해 {destination}에 안전하게 도착했습니다.', category: 'normal', advance: 'normal', detailScene: 'advance-clear', titleImageMode: 'same-as-detail-scene' }] } },
    ],
  },
