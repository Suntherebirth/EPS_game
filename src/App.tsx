...
import { chooseScenarioOption, getAvailableScenarioChoices, selectScenarioBattingEvent, startScenario } from './game/scenarioEngine'
...
function App() {
  ...
  const [scenario, setScenario] = useState<ScenarioState>(() => startScenario(OFFENSE_CORE_PACK, createScenarioContext(situation)))
  const [stats, setStats] = useState<Stats>({ runs: 0, hits: 0, outs: 0 })
  const [records, setRecords] = useState<RecordEntry[]>([])

  const restartAt = (nextSituation: Situation) => {
    setSituation(nextSituation)
    setScenario(startScenario(OFFENSE_CORE_PACK, createScenarioContext(nextSituation)))
    setPhase('playing')
  }
  ...
  const completeScenario = (state: ScenarioState) => {
    const result = state.context.records.join(' · ') || state.context.selectedLabel || '플레이 완료'
    setStats((current) => ({
  ...
  const chooseOption = (choiceId: string) => acceptState(chooseScenarioOption(OFFENSE_CORE_PACK, scenario, choiceId))
